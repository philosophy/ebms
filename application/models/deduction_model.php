...
class Deduction_model extends CI_Model {
    ...
    function getDeductions() {
        $sql = "SELECT * FROM deduction where active=? and company_id=?";
    ...
    function getDeductionDetails($id) {
    ...
    function createDeduction() {
        ...
        $this->db->trans_start();
        $sql = "INSERT INTO deduction (name, created_by, date_created, company_id) values (?, ?, ?, ?)";
        $this->db->query($sql,
            array(
                $this->get_name(),
                $this->get_created_by(),
        ...
        $sql = 'SELECT id from deduction where company_id = ? order by date_created desc limit 1';
        $query = $this->db->query($sql, array('company_id' => $this->get_company_id()));

        /* insert audit CREATE */
        parent::insertAuditTrail($this->get_created_by(), 1, $query->row()->id, lang('create_new_deduction'), $this->get_company_id(), $this->table_name);
    ...
    function deactivateDeduction() {
        ...
        /* insert audit DELETE */
        parent::insertAuditTrail($this->get_created_by(), 3, $this->get_id(), lang('deactivate_deduction'), $this->get_company_id(), $this->table_name);
    ...
    function updateDeduction() {
        ...
        /* insert audit UPDATE */
        parent::insertAuditTrail($this->get_last_updated_by(), 2, $this->get_id(), lang('update_deduction'), $this->get_company_id(), $this->table_name);
    ...
    function restoreDeduction() {
        ...
        /* insert audit */
        parent::insertAuditTrail($this->get_last_updated_by(), 2, $this->get_id(), lang('restore_deduction'), $this->get_company_id(), $this->table_name);
    ...
    function deductionExists() {
        $sql = "SELECT * FROM deduction where name = ? and company_id = ?";
    ...
    function recordExists() {
        $sql = "SELECT * FROM deduction where id!=? and name=? and company_id=?";
