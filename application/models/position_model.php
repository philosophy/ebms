...
class Position_model extends CI_Model {
    ...
    function getPositions() {
        $sql = "SELECT * FROM positions where active=? and company_id=?";
    ...
    function createPosition() {
        ...
        $this->db->trans_start();
        $sql = "INSERT INTO positions (name, created_by, date_created, company_id) values (?, ?, ?, ?)";
        $this->db->query($sql,
            array(
                $this->get_name(),
                $this->get_created_by(),
        ...
        $sql = 'SELECT id from positions where company_id = ? order by date_created desc limit 1';
    ...
    function positionExists() {
        $sql = "SELECT * FROM positions where name = ? and company_id = ?";
    ...
    function recordExists() {
        $sql = "SELECT * FROM positions where id!=? and name=? and company_id=?";
